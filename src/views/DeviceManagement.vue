...
const columnConfigs: ColumnConfig[] = [
  ...
  { key: 'updateTime_20', title: '更新时间', dataIndex: 'updateTime', width: 150, sorter: (a, b) => new Date(a.updateTime).getTime() - new Date(b.updateTime).getTime(), sortDirections: ['ascend', 'descend'], defaultSortOrder: 'descend' },
...
// Filtered and sorted data - uses Ant Design sorting when active, plain JavaScript default sorting when no sort is active
const filteredData = computed(() => {
  ...
  // Sorting logic
  if (sorterInfo.value && sorterInfo.value.order && sorterInfo.value.columnKey) {
    // Use Ant Design sorting when a sort is active
    ...
    // Default plain JavaScript sorting by update time in descending order (newest first)
...
const handleTableChange = (
  ...
  if (currentSorter && currentSorter.order) {
    ...
    // When sorting is cleared, revert to default (no sorting - will use plain JavaScript default)
    sorterInfo.value = {
      columnKey: null,
      order: null,
    };
