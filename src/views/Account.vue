...
console.log('Account.vue initial filter4GPlan:', filter4GPlan.value);
console.log('Account.vue initial filter4GPayment:', filter4GPayment.value);
...
const uniqueFourGPlans = computed(() => {
  ...
  plans.add('all');
  // Add unique 4G plans from the original data
  originalData.value.forEach(item => plans.add(item.fourGPlan));
  return [...plans];
});
...
const uniqueFourGPayments = computed(() => {
  ...
  payments.add('all');
  // Add unique 4G payment statuses from the original data
  originalData.value.forEach(item => payments.add(item.fourGPaymentStatus));
  return [...payments];
});
