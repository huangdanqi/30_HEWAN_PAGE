  ...
  <div class="product-management-header">
    <div class="filter-section">
      ...
      <div class="select-container">
        ...
        <a-select
          :value="filterIpName"
          @change="(newValue: string) => $emit('update:filterIpName', newValue)"
          style="width: 120px;"
        >
          <a-select-option value="all">全部</a-select-option>
          <!-- Options will be dynamically populated from props -->
          <a-select-option v-for="ip in uniqueIpNames" :key="ip" :value="ip">{{ ip }}</a-select-option>
        </a-select>
      </div>
    </div>
    ...
    <div class="action-section">
      ...
      />
      <a-button type="primary" @click="$emit('create-product')" style="margin-right: 8px;">新建产品</a-button>
...
defineProps({
  ...
  uniqueIpNames: { type: Array as () => string[], default: () => [] },
});
